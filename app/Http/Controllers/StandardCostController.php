...
class StandardCostController extends Controller
{
    public function update(Request $request)
    {
        ...
        foreach ($groups as $group) {
            ...
            $disc_price = ceil((($group->disc->standardMaterial->price ?? 0) * $group->disc?->quantity ?? 0) * 100) / 100;
            $rim_price = ceil((($group->rim->standardMaterial->price ?? 0) * $group->rim?->quantity ?? 0) * 100) / 100;
            $sidering_price = ceil((($group->sidering->standardMaterial->price ?? 0) * $group->sidering?->quantity ?? 0) * 100) / 100;
        ...
        return redirect()->route('sc.report');
    }

    public function preview(Request $request, string $item_code)
    {
        // $previewData = StandardCost::where('item_code', $item_code)->with('bom')->first();
        $opex = $request->query('opex');
        $progin = $request->query('progin');
        $previewData = StandardCost::where('item_code', $item_code)
            ->with([
